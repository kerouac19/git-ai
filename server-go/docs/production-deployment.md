...
- [ ] （启用 `git-ai upgrade` 时）`RELEASE_UPLOAD_TOKEN` 已生成，`/opt/git-ai/releases` 已创建并 `chown git-ai`
- [ ] （启用 `git-ai upgrade` 时）`sync-releases.sh` 调度方式已确定（systemd timer / cron），首次手工冒烟通过
...
sudo chown -R git-ai:git-ai /opt/git-ai/logs /opt/git-ai/releases
...
# ───── Release 同步（可选，启用 /api/releases 与 git-ai upgrade 时必须） ─────
RELEASE_STORAGE_PATH=/opt/git-ai/releases      # 默认值；若改路径需同步更新 systemd ReadWritePaths 和目录 owner
RELEASE_UPLOAD_TOKEN=<openssl rand -base64 48 的输出>   # 不设则上传端点返回 503
PRIVATE_SERVER=https://git-ai.your-company.com          # sync-releases.sh 使用；可单独放 sync 专用 env
# GITHUB_TOKEN=<PAT，无则走匿名限流（60 次/小时/IP）>
# GITHUB_REPO=git-ai-project/git-ai            # 如 fork 则覆盖
```
...
ProtectHome=yes
ReadWritePaths=/opt/git-ai/logs /opt/git-ai/releases
...
### 5.4 Release 同步（sync-releases.sh）

`scripts/sync-releases.sh` 负责把 GitHub Releases 上的 `SHA256SUMS`、`install.sh`、`install.ps1` 拉下来，再通过 `PUT /api/releases/:channel/...` 推送到私有服务器的 `RELEASE_STORAGE_PATH`，并写入 `current.json` 作为版本指针。脚本是**幂等**的（`current.json.tag` 已等于 GitHub 最新 tag 时会 no-op），**不会**被 server-go 自动触发，必须外部调度，否则客户端 `git-ai upgrade` 永远看不到新版本。
...
# 放置脚本（服务用户只读执行即可）
sudo install -o git-ai -g git-ai -m 0755 \
    server-go/scripts/sync-releases.sh \
    /opt/git-ai/scripts/sync-releases.sh

# 存储目录（若 .env 中 RELEASE_STORAGE_PATH 指向其它目录，按相同方式 mkdir + chown）
sudo mkdir -p /opt/git-ai/releases
sudo chown git-ai:git-ai /opt/git-ai/releases
sudo chmod 750 /opt/git-ai/releases
```

若存储路径不是默认的 `/opt/git-ai/releases`，务必把实际路径加入 systemd 单元的 `ReadWritePaths`，否则写入会被 `ProtectSystem=strict` 拦截。

#### 5.4.2 首次手工冒烟

部署后、启用调度前，务必手工跑一次，验证 token、Bearer 前缀、网络与落盘：

```bash
# 用服务用户身份跑，确认权限一致
sudo -u git-ai env $(grep -v '^#' /opt/git-ai/.env | xargs) \
     /opt/git-ai/scripts/sync-releases.sh
...
预期：两个 channel（`latest` / `next`）各打印 `synced <none> -> vX.Y.Z` 或 `already at ...`；失败时先核对 `Authorization: Bearer <token>`、`PRIVATE_SERVER` 与 Nginx `client_max_body_size` 是否 >= 预期制品体积。

#### 5.4.3 调度（二选一）

**方案 A：systemd timer（推荐，与服务统一管理）**
...
```cron
*/15 * * * * git-ai PRIVATE_SERVER=https://git-ai.your-company.com \
             RELEASE_UPLOAD_TOKEN=xxxx \
             /opt/git-ai/scripts/sync-releases.sh \
             >> /var/log/git-ai-sync.log 2>&1
```

频率建议 5–15 分钟。匿名 GitHub API 每小时 60 次，多节点同步或频率较高时**必须**设 `GITHUB_TOKEN` 以免限流。

#### 5.4.4 常见问题

| 现象 | 排查方向 |
|------|----------|
| 所有 PUT 返回 401 | Header 必须是 `Authorization: Bearer <token>`；裸 token 或其它 scheme 一律拒绝 |
| 所有 PUT 返回 503 | server-go 端 `RELEASE_UPLOAD_TOKEN` 未设，功能软关闭 |
| journalctl 报 `permission denied … /opt/git-ai/releases` | 目录 owner 不是 `git-ai` 或未加入 `ReadWritePaths` |
| `403 rate limit exceeded` | 设置 `GITHUB_TOKEN`（PAT，只需 `public_repo` 读取） |
| 客户端 `git-ai upgrade` 提示 checksum 不匹配且重试后恢复 | promote race（首版接受，文档见 releases-upgrade-plan.md §风险） |

#### 5.4.5 回滚

- **暂停同步**：`systemctl disable --now git-ai-sync.timer`，私服保持当前版本不再变更。
- **关闭上传端点**：从 `.env` 移除 `RELEASE_UPLOAD_TOKEN` 并重启 `git-ai.service`，所有 `PUT /api/releases/...` 立刻返回 503，不影响其他 API。
- **回退到旧版本**：手工编辑 `/opt/git-ai/releases/:channel/current.json` 指回旧 tag（该 tag 的 artifacts 目录需仍然存在）。
...
server {
    ...
    # Release 制品上传（PUT，二进制），放宽 body 限制并禁用缓存
